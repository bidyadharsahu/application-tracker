...
export default function JobCard({ job, admin = false, onToggle, onEdit, onDelete }) {
  ...
  const accent = isApplied ? "var(--c-green)" : isUrgent ? "var(--c-red)" : isNotStarted ? "var(--c-purple)" : "var(--c-amber)";

  return (
    <article data-testid={`job-card-${job.id}`} className="card"
      style={{ overflow: "hidden", borderTop: `3px solid ${accent}` }}>
      <div style={{ padding: "16px 16px 14px" }}>

        {/* Title + pill */}
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 10, marginBottom: 10 }}>
          <h3 data-testid={`job-title-${job.id}`}
            style={{ fontSize: 16, fontWeight: 700, color: "var(--t1)", margin: 0, lineHeight: 1.35, flex: 1 }}>
            {job.job_name}
          </h3>
          <span data-testid={`status-stamp-${job.id}`}
            className={`pill ${isApplied ? "pill-applied" : isUrgent ? "pill-urgent" : isNotStarted ? "pill-notice" : "pill-pending"}`}
            style={{ flexShrink: 0 }}>
        ...
        {job.tags && (
          <div className="scroll-x" style={{ marginBottom: 10 }}>
            {job.tags.split(",").map((t, i) => t.trim() && (
              <span key={i} style={{ display: "inline-flex", alignItems: "center", gap: 3, background: "#EEF2FF", border: "1px solid #C7D2FE", color: "var(--brand)", padding: "3px 9px", borderRadius: 99, fontSize: 12, fontWeight: 600, whiteSpace: "nowrap", flexShrink: 0 }}>
                <Tag size={9} />{t.trim()}
              </span>
            ))}
          </div>
        ...
        {job.source && job.source !== "manual" && (
          <div style={{ background: "var(--c-green-bg)", borderRadius: 10, padding: "8px 12px", marginBottom: 10 }}>
            <div style={{ fontSize: 12, fontWeight: 700, color: "var(--c-green)" }}>🤖 Auto-found · {job.match_score}% match</div>
            {job.match_reason && <div style={{ fontSize: 12, color: "var(--t3)", marginTop: 2 }}>{job.match_reason}</div>}
          </div>
        )}
        ...
        <div style={{ display: "grid", gridTemplateColumns: isApplied ? "1fr" : "1fr 1fr", gap: 8, marginBottom: 10 }}>
          {!isApplied && (
            <DateCell icon={<CalendarDays size={13} />} label="Last Date"
              ...
              subColor={deadlineDays !== null && deadlineDays <= 1 ? "var(--c-red)" : "var(--c-amber)"}
          ...
          <DateCell icon={<FileText size={13} />} label="Exam Date"
            ...
            subColor={examDays !== null && examDays <= 1 ? "var(--c-red)" : examDays !== null && examDays <= 7 ? "var(--c-amber)" : "var(--t3)"}
            testId={`job-exam-date-${job.id}`} />
        </div>

        {!isApplied && job.last_date && (
          <div style={{ marginBottom: 10 }}><Countdown targetDate={job.last_date} /></div>
        )}

        {job.notes && (
          <div data-testid={`job-notes-${job.id}`} style={{ background: "var(--surface-2)", borderRadius: 10, padding: "10px 12px", marginBottom: 10, fontSize: 14, color: "var(--t2)", lineHeight: 1.55, borderLeft: "3px solid var(--border)" }}>
            {job.notes}
          </div>
        )}

        {(job.app_username || job.app_password) && (
          <div style={{ background: "#EEF2FF", borderRadius: 10, padding: "10px 12px", marginBottom: 10 }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: "var(--brand)", textTransform: "uppercase", letterSpacing: ".06em", marginBottom: 7 }}>Login</div>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 7 }}>
              {job.app_username && (
                <button type="button" onClick={() => copy(job.app_username, "Username")}
                  style={{ display: "flex", alignItems: "center", gap: 5, background: "#fff", border: "1px solid #C7D2FE", borderRadius: 8, padding: "7px 11px", fontSize: 14, fontWeight: 600, color: "var(--t1)", cursor: "pointer" }}>
                  <User size={13} color="var(--brand)" />{job.app_username}<Copy size={11} color="var(--t4)" />
                </button>
              )}
              {job.app_password && (
                <button type="button" onClick={() => copy(job.app_password, "Password")}
                  style={{ display: "flex", alignItems: "center", gap: 5, background: "#fff", border: "1px solid #C7D2FE", borderRadius: 8, padding: "7px 11px", fontSize: 14, fontWeight: 600, color: "var(--t1)", cursor: "pointer" }}>
                  <Lock size={13} color="var(--brand)" />{job.app_password}<Copy size={11} color="var(--t4)" />
                </button>
              )}
            </div>
          </div>
        ...
        <div style={{ marginBottom: 12 }}>
          <button type="button" onClick={() => setDocsOpen(p => !p)}
            style={{ display: "flex", alignItems: "center", gap: 8, width: "100%", background: "var(--surface-3)", border: "none", borderRadius: 10, padding: "10px 14px", fontSize: 14, fontWeight: 600, color: "var(--t2)", cursor: "pointer" }}>
            <Paperclip size={15} color="var(--t3)" />
            <span>Documents</span>
            {docs.length > 0 && <span style={{ background: "var(--brand)", color: "#fff", borderRadius: 99, padding: "0 7px", fontSize: 11, fontWeight: 700, marginLeft: 2 }}>{docs.length}</span>}
            <span style={{ marginLeft: "auto" }}>{docsOpen ? <ChevronUp size={14} /> : <ChevronDown size={14} />}</span>
          </button>
          {docsOpen && (
            <div className="a-in" style={{ background: "var(--surface-2)", borderRadius: "0 0 10px 10px", padding: 12 }}>
              <div className="scroll-x" style={{ marginBottom: docs.length > 0 ? 10 : 0 }}>
                {["admit_card", "hall_ticket", "result", "other"].map(type => (
                  <label key={type} style={{ display: "inline-flex", alignItems: "center", gap: 5, background: "#fff", border: "1.5px solid var(--border)", borderRadius: 99, padding: "7px 13px", fontSize: 13, fontWeight: 600, color: "var(--t2)", cursor: "pointer", flexShrink: 0, opacity: uploading ? .5 : 1 }}>
                    📎 {type.replace("_", " ")}
              ...
              {docs.map(doc => (
                <div key={doc.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "9px 0", borderTop: "1px solid var(--border)" }}>
                  ...
                  <button type="button" onClick={() => openDoc(doc.file_path)}
                    style={{ flex: 1, textAlign: "left", background: "none", border: "none", fontSize: 14, fontWeight: 600, color: "var(--brand)", cursor: "pointer", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", padding: 0 }}>
                    {doc.document_type?.replace("_", " ")} — {doc.file_name}
                  </button>
                  <button type="button" onClick={() => deleteDoc(doc)}
                    style={{ background: "none", border: "none", color: "var(--c-red)", cursor: "pointer", padding: 4, display: "flex", flexShrink: 0 }}>
                    <Trash2 size={14} />
                  </button>
                </div>
              ))}
        ...
        <div style={{ display: "flex", gap: 8 }}>
          {isFuture ? (
            <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", background: "var(--surface-3)", borderRadius: 99, padding: "11px", fontSize: 14, fontWeight: 600, color: "var(--t3)" }}>
              Opens {formatDate(job.start_date)}
            </div>
          ) : (
            <a href={job.apply_link} target="_blank" rel="noopener noreferrer"
              data-testid={`apply-link-${job.id}`}
              className="btn btn-brand" style={{ flex: 1, textDecoration: "none" }}>
          ...
          {(!isApplied || admin) && (
            <button type="button" onClick={() => onToggle && onToggle(job)}
              ...
              style={{ flexShrink: 0, color: isApplied ? "var(--c-green)" : undefined }}>
              {isApplied ? <CheckCircle2 size={16} /> : <Circle size={16} />}
              {isApplied ? "Applied" : "Mark"}
            </button>
          )}
          {admin && <>
            <button type="button" onClick={() => onEdit && onEdit(job)} data-testid={`edit-job-${job.id}`} className="btn btn-ghost btn-sq"><Pencil size={16} /></button>
            <button type="button" onClick={() => onDelete && onDelete(job)} data-testid={`delete-job-${job.id}`} className="btn btn-danger-soft btn-sq"><Trash2 size={16} /></button>
          </>}
        </div>
      </div>
    </article>
...
function DateCell({ icon, label, value, sub, subColor, testId }) {
  return (
    <div style={{ background: "var(--surface-2)", borderRadius: 10, padding: "10px 12px" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 11, fontWeight: 600, color: "var(--t4)", textTransform: "uppercase", letterSpacing: ".06em", marginBottom: 4 }}>{icon}{label}</div>
      <div style={{ fontSize: 15, fontWeight: 700, color: "var(--t1)" }} data-testid={testId}>{value}</div>
